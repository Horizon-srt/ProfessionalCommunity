...
type provideMethod = 'GET' | 'POST' | 'PATCH'| 'DELETE'
const getToken = () => {
...
export default function useFetch(params: {url: string, method: provideMethod, params: any}) {
    const {url, method, params:fetchParams} = params
    const {data,error,isLoading,mutate } = useSWR([url,method,fetchParams], ([url,method,fetchParams])=>dataFetcher(url,method,fetchParams))
